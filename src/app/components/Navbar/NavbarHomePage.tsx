...
const NavbarHomePage = () => {
  ...
  const handleScroll = () => {
    if (window.scrollY > 50) { // Możesz dostosować wartość progową
      setScrolled(true);
    } else {
      setScrolled(false);
    }
  ...
  return (
    <div className={`navbar fixed ${scrolled ? 'navbar-scrolled bg-[#141a1e] text-white' : 'top-10 bg-base-100 max-w-screen-xl left-1/2 transform -translate-x-1/2'} left-0 right-0 z-50 transition-all duration-300`}>
      ...
      <div className="navbar-center">
        <Image
          src="/logo/logo.png" // Ścieżka do logo
          alt="Logo"
          width={75} // Szerokość logo
          height={75} // Wysokość logo
          className="object-contain" // Upewnij się, że obrazek jest w pełni widoczny
        />
      </div>
